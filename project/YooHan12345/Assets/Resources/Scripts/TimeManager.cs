...
public class TimeManager : MonoBehaviour {
	...
	void Update () {
        ...
        if (time < 600f) timecnt();     //완성용: time < 600f
        ...
        if (time > waitingTime)
        {
            ...
            GetComponent<CameraShake>().SendMessage("setShake", 10.0f);  // 8초 동안 카메라 효과
            while (randnum > 0) //바닥에 빵꾸 1~3개 남
            {
                GetComponent<FloorManager>().SendMessage("Disturb", null);
				GetComponent<CeilingManager>().SendMessage("Disturb", null);
				randnum--;
            }
